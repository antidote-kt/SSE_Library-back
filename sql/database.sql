...
CREATE TABLE favorites (
   ...
   favorite_id BIGINT UNSIGNED NOT NULL COMMENT '被收藏对象ID',
   favorite_type varchar(20) NOT NULL COMMENT '被收藏对象类型',
   created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '收藏时间',
   updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
   deleted_at TIMESTAMP NULL DEFAULT NULL COMMENT '软删除标记，（NULL表示未删除）',
   PRIMARY KEY (id),
   UNIQUE KEY uk_user_favorite (user_id, favorite_id, favorite_type),
   KEY idx_favorite (favorite_id, favorite_type)
) COMMENT='文档收藏记录表';
...
CREATE TABLE documents (
   ...
   author VARCHAR(100) COMMENT '作者姓名，文档的创作者信息',
   uploader_id BIGINT UNSIGNED NOT NULL COMMENT '上传者用户ID，关联users表，记录谁上传了此文档',
   course_id BIGINT UNSIGNED NOT NULL COMMENT '文档所属课程ID',
...
CREATE TABLE courses (
     id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT '课程唯一标识ID',
     title VARCHAR(200) NOT NULL COMMENT '课程标题',
     description TEXT COMMENT '课程描述信息',
     instructor varchar(50) NOT NULL COMMENT '记录课程的授课讲师',
     category_id BIGINT UNSIGNED COMMENT '课程所属分类',
...
CREATE TABLE categories (
    id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT '分类唯一标识ID',
    name VARCHAR(50) NOT NULL COMMENT '分类名称',
    description TEXT COMMENT '分类描述，详细说明分类的用途和包含的内容',
    parent_id BIGINT UNSIGNED DEFAULT NULL COMMENT '父分类ID，关联本表，用于实现层级分类，NULL表示顶级分类',
